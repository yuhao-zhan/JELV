...
def main():
    parser = argparse.ArgumentParser(
        ...
        usage="%(prog)s [-h] [options] -ref REF -hyp HYP -alpha alpha -method TS/EW",
    ...
    parser.add_argument("--alpha", help="alpha for generalized F-score", required=True)
    # parser.add_argument("--method", help="EW or TS", required=True)
    ...
    parser.add_argument(
        "--scorer", default="independent", choices=["dependent", "independent"],
    ...
    # Evaluate pipeline
    score, results = metric.evaluate(dataset_hyp, dataset_ref, alpha)
    f_score = score.get('F')
    
    # print(score)
    
    # Writing the sorted F-scores to a file
    # with open(f'metric_score/CLEME_Ind_sys/sorted_f_scores_alpha_{alpha}.txt', 'a') as f:
    #     f.write(f"{os.path.basename(args.hyp)}: {f_score}\n")


    # print(f"{args.hyp}:  F-scores have been sorted and written to 'sorted_f_scores.txt'.")
