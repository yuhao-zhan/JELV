...
class CorpusScorer(Scorer):
    def __call__(self, scorer_inputs: List[List[Dict]], alpha) -> Dict[str, Any]:
        """ Calculate corpus-level F_beta Score """
        # total_tp, total_fp, total_fn, total_tn = 0, 0, 0, 0
        total_tp, total_fp, total_fn, total_tn, total_OC = 0, 0, 0, 0, 0
        for index, sample_result in enumerate(scorer_inputs):
            #print('-' * 50)
            # print(f"Sentence {index+1}")
            # best_f, best_tp, best_fp, best_fn, best_tn = -1.0, 0, 0, 0, 0
            best_f, best_tp, best_fp, best_fn, best_tn, best_OC, best_modified_f, best_OC_Score = -1.0, 0, 0, 0, 0, 0, 0, 0
            for ref_result in sample_result:
                ...
                _p, _r, _f, _modified_f, _OC_Score = compute_generalized_f(total_tp + _tp, total_fp + _fp, total_fn + _fn, alpha, total_OC + _OC)
                #print(f"_f: {_f}. best_f: {best_f}")
                if (_f > best_f) or \
                        ...
                        (_f == best_f and _tp == best_tp and _fp == best_fp and _fn == best_fn and _tn > best_tn) or \
                        (_f == best_f and _tp == best_tp and _fp == best_fp and _fn == best_fn and _tn == best_tn and _OC < best_OC):
                    best_f, best_tp, best_fp, best_fn, best_tn, best_OC, best_modified_f, best_OC_Score  = _f, _tp, _fp, _fn, _tn, _OC, _modified_f, _OC_Score
            total_tp += best_tp
            total_fp += best_fp
            total_fn += best_fn
            total_tn += best_tn
            total_OC += best_OC
        p, r, f, modified_f, OC_Score = compute_generalized_f(total_tp, total_fp, total_fn, alpha, total_OC)
        ...
        return {
            ...
            KEY_F: f, KEY_MODIFIED_F: modified_f, KEY_OC_SCORE: OC_Score, KEY_ACC: acc, KEY_P: p, KEY_R: r,
            KEY_TP: round(total_tp, 2), KEY_FP: round(total_fp+total_OC, 2),
            KEY_FN: round(total_fn, 2), KEY_TN: round(total_tn, 2),
            KEY_OC: round(total_OC, 2), KEY_OC: round(total_OC, 2),
        }
